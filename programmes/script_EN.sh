...
do
    ...
    then
		...
		nb_occurrence=$(cat ../dumps-text/"${langue}-${lineno}.txt"|grep -P -i -o "game\b|games\b"|wc -l)

        #récupérer les contextes
        contexte=$(cat ../dumps-text/${langue}-${lineno}.txt|grep -P -i -C1 "game\b|games\b")
        ...
        echo -e "		<tr>
                    ...
                    <td>$encodage</td>
                    <td>$nb_occurrence</td>
                    <td><a href="${page_aspiree}">EN-${lineno}</td>
                    <td><a href="${dump_texte}">EN-${lineno}</td>
                    <td><a href="${contextes}">EN-${lineno}</td>
