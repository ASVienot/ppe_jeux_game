...
do
    ...
    then
		...
		nb_occurrence=$(cat ../dumps-text/"${langue}-${lineno}.txt"|grep -P -i -o "jeu(.)?\b|jeu(.)?-"|wc -l)

        #récupérer les contextes
        contexte=$(cat ../dumps-text/${langue}-${lineno}.txt|grep -P -i -w -C1 "jeu(.)?\b|jeu(.)?-")
        ...
        echo -e "		<tr>
                    ...
                    <td>$encodage</td>
                    <td>$nb_occurrence</td>
                    <td><a href="${page_aspiree}">FR-${lineno}</td>
                    <td><a href="${dump_texte}">FR-${lineno}</td>
                    <td><a href="${contextes}">FR-${lineno}</td>
